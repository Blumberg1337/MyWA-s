...
-- Data Model
-- [Class-Role] = {{paladinBlessings}, classFileName, talentTreeNumber, talentNumber}
...
aura_env.sendPaladinBlessingEvents = function()
  ...
  if (classFilename == "HUNTER") then
    -- Check for Expose Weakness -> SV
    local _, _, _, _, rank = GetTalentInfo(3,21)
    if (rank > 0) then
      for i=1, i <= aura_env.paladinCount do
        if (sanctuaryAndKingsAvailability) then
          if (aura_env.defaultBuffPriorities["HUNTER-SV"][i] == "SANCTUARY" or
              aura_env.defaultBuffPriorities["HUNTER-SV"][i] == "KINGS") then
            WeakAuras.ScanEvent(customEvent..aura_env.defaultBuffPriorities["HUNTER-SV"][i])
          end
        elseif (kingsAvailability or sanctuaryAvailability) then
          if (kingsAvailability and aura_env.defaultBuffPriorities["HUNTER-SV"][i] == "KINGS") then
            kingsPriority = i
            if (sanctuaryPriority and kingsPriority < sanctuaryPriority) then
              WeakAuras.ScanEvent(customEvent..aura_env.defaultBuffPriorities["HUNTER-SV"][i])
            end
          end
          if (sanctuaryAvailability and aura_env.defaultBuffPriorities["HUNTER-SV"][i] == "SANCTUARY") then
            sanctuaryPriority = i
      ...
      for i=1, i <= aura_env.paladinCount do
        if (sanctuaryAndKingsAvailability) then
          if (aura_env.defaultBuffPriorities["HUNTER-BM"][i] == "SANCTUARY" or
              aura_env.defaultBuffPriorities["HUNTER-BM"][i] == "KINGS") then
            WeakAuras.ScanEvent(customEvent..aura_env.defaultBuffPriorities["HUNTER-BM"][i])
          end
        elseif (kingsAvailability or sanctuaryAvailability) then
          if (kingsAvailability and aura_env.defaultBuffPriorities["HUNTER-BM"][i] == "KINGS") then
            kingsPriority = i
            if (sanctuaryPriority and kingsPriority < sanctuaryPriority) then
              WeakAuras.ScanEvent(customEvent..aura_env.defaultBuffPriorities["HUNTER-BM"][i])
            end
          end
          if (sanctuaryAvailability and aura_env.defaultBuffPriorities["HUNTER-BM"][i] == "SANCTUARY") then
            sanctuaryPriority = i
            if (kingsPriority and sanctuaryPriority < kingsPriority) then
              WeakAuras.ScanEvent(customEvent..aura_env.defaultBuffPriorities["HUNTER-BM"][i])
            end
